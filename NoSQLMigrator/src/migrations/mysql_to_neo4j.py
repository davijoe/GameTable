...
class MySQLToNeo4jMigration:
    ...
    def migrate_games_and_related(self):
        ...
        try:
            ...
            logger.info(f"Found {len(games)} games, {len(designers)} designers, {len(artists)} artists, {len(genres)} genres, {len(publishers)} publishers, {len(mechanics)} mechanics, {len(videos)} videos")
            ...
            relationship_queries = Neo4jTransformer.create_game_relationships(
                game_designers, game_artists, game_genres, game_publishers, game_mechanics
            )
        ...
        for i, query_data in enumerate(queries):
            try:
                if isinstance(query_data, dict) and 'query' in query_data:
                    ...
                    logger.warning(f"Invalid query data at index {i} for {description}")
            except Exception as e:
                logger.warning(f"Error executing query {i} for {description}: {e}")
