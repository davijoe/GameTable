...
class MySQLToMongoDBMigration:
        ...
        try:
            ...
            for game in games_data:
                try:
                    ...
                    publisher_list = [
                        {"id": p["id"], "name": p["name"]} for p in publishers
                    ]
                    mechanic_list = [
                        {"id": m["id"], "name": m["name"]} for m in mechanics
                    ]
                    ...
                    game_document["metadata"]["migrated_at"] = self._serialize_datetime(
                        datetime.utcnow()
                    )

                    self.mongodb_conn.insert_documents(
                        "games",
                        [game_document],
                    )
                    logger.info(
                        f"Migrated game: {game['name']} (ID: {game_id}, Reviews: {
                            len(review_ids)
                        })"
                    )
                    ...
                    logger.error(
                        f"Error migrating game {game.get('id', 'unknown')}: {e}"
                    )
                    continue
        ...
        try:
            ...
            for review in reviews:
                try:
                    ...
                    review_doc["metadata"]["migrated_at"] = self._serialize_datetime(
                        datetime.utcnow()
                    )
                    ...
                    if len(review_documents) >= 1000:
                        ...
                        logger.info(
                            f"Migrated batch of {len(review_documents)} reviews"
                        )
                        review_documents = []

                except Exception as e:
                    logger.error(
                        f"Error migrating review {review.get('id', 'unknown')}: {e}"
                    )
                    continue
    ...
    def _update_game_ratings(self):
        ...
        try:
            pipeline = [
                {"$group": {"_id": "$game_id", "avg_rating": {"$avg": "$star_amount"}}}
            ]
            ...
            for result in results:
                ...
                games_collection.update_one(
                    {"_id": game_id},
                    {"$set": {"ratings.average_user_rating": avg_rating}},
                )
