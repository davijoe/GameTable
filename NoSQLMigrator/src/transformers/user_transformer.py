...
class UserTransformer:
    ...
    def transform_user_data(
        ...
        for friendship in friendships:
            friend_id = (
                ...
                if friendship["user_id_1"] == user_data["id"]
                else friendship["user_id_1"]
            )
            friend_data = {
                "user_id": friend_id,
                "status": friendship["status"],
                "friends_since": UserTransformer._serialize_date(
                    friendship["created_at"]
                ),
                "last_updated": UserTransformer._serialize_date(
        ...
        user_document = {
            ...
            "metadata": {"source_id": user_data["id"], "migrated_at": None},
        }
    ...
    def _build_conversations(user_id, all_messages):
        ...
        for message in all_messages:
            other_user_id = (
                message["user_id_2"]
                if message["user_id_1"] == user_id
                else message["user_id_1"]
            )
            conversation_key = (
                f"convo_{min(user_id, other_user_id)}_{(max(user_id, other_user_id))}"
            )
            ...
            message_data = {
                ...
                "text": message["text"],
                "timestamp": UserTransformer._serialize_date(
                    message["timestamp"],
                ),
            }
            ...
            if (
                conversations[conversation_key]["_last_message_timestamp"] is None
                or message["timestamp"]
                > conversations[conversation_key]["_last_message_timestamp"]
            ):
                conversations[conversation_key]["_last_message_timestamp"] = message[
                    "timestamp"
                ]
                conversations[conversation_key]["last_message"] = (
                    UserTransformer._serialize_date(message["timestamp"])
                )

        conversation_list = list(conversations.values())
        conversation_list.sort(
            key=lambda x: x["_last_message_timestamp"] or datetime.min, reverse=True
        )
